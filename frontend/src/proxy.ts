...
export async function proxy(request: NextRequest) {
    ...
    // 1. User coming from email reset link (has userId + token in query params)
    ...
    if (pathname === "/reset-password") {
        ...
        // Case 2: Coming from email reset link (has userId and token)
        if (userId && token) {
            try {
                // Verify the token
                ...
                if (verifiedToken.success && verifiedToken.payload!.userId !== userId) {
                    return NextResponse.redirect(new URL('/forgot-password?error=invalid-link', request.url));
                }

                // Token and userId are valid, allow access without authentication
                return NextResponse.next();
            } catch {
